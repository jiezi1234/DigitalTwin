...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--persist-dir",
        default=os.path.join(Config.PROJECT_ROOT, "chroma_db_mm"),
        help="Chroma 持久化目录",
    )
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument(
        "--textbook-file",
        default=os.path.join(Config.PROJECT_ROOT, "data/pdf/textbook.pdf"),
...
def main() -> None:
    ...
    _validate_files(args.notes_files + [args.textbook_file])
    ...
    mm_results = []
    total_steps = len(args.notes_files) + 1
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, args.pdf_workers)) as executor:
        future_to_pdf = {
            ...
            for idx, pdf_path in enumerate(args.notes_files)
        }
    ...
    ocr_bar = None
    if progress:
        progress.set_postfix_str(f"OCR: {os.path.basename(args.textbook_file)}")
        ...
        with progress_lock:
            if event.get("event") == "ocr_start":
                ...
                ocr_bar = tqdm(
                    ...
                    desc=os.path.basename(args.textbook_file),
                    unit="page",
                    position=len(args.notes_files) + 1,
                    leave=True,
                )
    ...
    ocr_documents = _build_documents_from_pages(
        ...
        filename=os.path.basename(args.textbook_file),
    )
