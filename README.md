...
- 结构化 PDF：根据标题、段落、列表、图注和表格边界切分教材，记录字号、版面框、父页面和切分策略；图片按版面距离关联图注与附近正文。
- 回答可信度：按文本/图片置信度和最少证据数执行可校准的回答门禁；进一步检查事实句引用覆盖率及引用证据支持度。
- 时间邻域扩展：语义命中聊天消息后，按会话 ID 和消息序号补齐前后消息，恢复完整对话语境。
- 教材问答：并行召回多模态文本、OCR 文本、BM25 关键词结果和教材图片；文本候选使用加权 RRF 融合并统一重排。
...
1. PDF 导入阶段提取带字体和坐标的文本块、表格和页面图片；按结构边界合并文本块，并为图片关联图注和附近正文。
2. 查询阶段使用教材领域改写，并召回多模态文本、OCR 文本、BM25 关键词结果和图片。
3. 三个文本通道的分数不可直接比较，因此使用加权 Reciprocal Rank Fusion 合并，再统一重排 Top-N 候选。
4. 文本上下文经过预算和去重后返回实际入选结果，保证 `[n]` 与来源页码保持一致。
5. 分别使用文本召回通道的原始相似度和图片相似度执行证据门禁；达到模态阈值和最少证据数才调用生成模型，否则直接返回“教材资料不足”。
6. 命中图片作为多模态输入发送给视觉语言模型；`quality` 同时返回证据门禁、引用编号、事实句引用覆盖率和引用内容支持度。

OCR 通道不可用时会降级为多模态文本与图片检索，不让单个外部调用中断整条问答链路。

结构化切分会改变文档 ID。升级后必须重新构建教材索引，避免新旧切分结果混在同一 collection：
...
默认计算引用编号正确率、事实句引用覆盖率、引用支持准确率、完整支撑回答率、拒答准确率和答案关键词召回；增加
`--llm-judge` 后才会额外调用评测模型计算忠实度。评测器不会在线修改答案或触发重试。

证据阈值不要直接使用示例默认值作为实验结论。收集“足以回答”的正例和同主题错误页面、视觉相似错误图片等困难负例后运行：
...
报告会分别给出 `TUTOR_MIN_TEXT_EVIDENCE_SCORE` 与 `TUTOR_MIN_IMAGE_EVIDENCE_SCORE` 建议值，并列出 Precision、Recall、F1 与误放行率。将人工复核后的阈值写入 `.env`，才能形成可复现的“证据不足拒答”指标。
