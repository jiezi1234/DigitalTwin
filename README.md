...
- 回答可信度：教材无有效证据时直接拒答；有证据时校验 `[n]` 引用是否越界，并支持离线忠实度、引用覆盖率与拒答准确率评测。
...
5. 文本和图片均无命中时直接返回“教材资料不足”，不调用生成模型。
6. 命中图片作为多模态输入发送给视觉语言模型，回答中的图片标记由前端替换为实际图片；文本引用会被校验并随 `quality` 字段返回有效和无效编号。
