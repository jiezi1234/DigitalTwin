...
- 回答可信度：按文本/图片置信度和最少证据数执行可校准的回答门禁；证据不足时直接拒答，有证据时校验 `[n]` 引用是否越界，并支持离线忠实度、引用覆盖率与拒答准确率评测。
...
6. 命中图片作为多模态输入发送给视觉语言模型，回答中的图片标记由前端替换为实际图片；证据判定明细和文本引用校验结果随 `quality` 字段返回。
...
默认只计算确定性的引用正确率、引用覆盖率、拒答准确率和答案关键词召回；增加
