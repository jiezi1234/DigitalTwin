...
def test_wechat_csv_loader_filtering():
    ...
    with tempfile.NamedTemporaryFile(
        ...
        writer = csv.DictWriter(
            f, fieldnames=["msg", "talker", "is_sender", "room_name"]
        )
        writer.writeheader()
    ...
    try:
        ...
        assert docs[0].metadata["conversation_id"].endswith("::direct")
        assert docs[0].metadata["message_index"] == 0
        assert docs[1].metadata["message_index"] == 1
        assert docs[2].metadata["conversation_id"].endswith("::group1")
        assert docs[2].metadata["message_index"] == 0
    finally:
...
def test_dataloader_batch_and_incremental():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 1. 第一轮加载
        docs = WeChatCSVLoader.load_batch(
            os.path.join(tmpdir, "*.csv"), incremental=True, tracking_file=tracking_path
        )
        assert len(docs) == 2

        # 2. 第二轮加载 (无新文件)
        docs2 = WeChatCSVLoader.load_batch(
            os.path.join(tmpdir, "*.csv"), incremental=True, tracking_file=tracking_path
        )
        assert len(docs2) == 0
        ...
        docs3 = WeChatCSVLoader.load_batch(
            os.path.join(tmpdir, "*.csv"), incremental=True, tracking_file=tracking_path
        )
        assert len(docs3) == 1
