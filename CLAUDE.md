...
- 教材问答使用多模态文本、OCR、BM25 和图片召回，三个文本排序通过加权 RRF 融合后统一重排
- PDF 教材按标题、段落、列表、图注和表格边界切分，结构块记录父页面、版面框和切分版本；图片按坐标关联图注与附近正文
...
- 对生成答案执行引用编号与事实句证据支持校验；LLM 忠实度判断仅用于离线评测
...
│   │   ├── pdf_chunker.py             # 标题/段落/表格感知的教材切分
